# -*- coding: utf-8 -*-
"""
Streamlit App: 上架盘点表生成器（含差异储位剔除 + 随机抽样）

功能：
- 上传 1) 前一日上架结果表  2) 差异明细表
- 选择每个上架员抽样条数 k（可复现的随机种子 seed）
- 导出：仅下载【上架盘点表】（单 Sheet）

运行：
1) pip install streamlit pandas openpyxl numpy
2) streamlit run putaway_check_app_only_checktable_v2.py
"""
...
def sample_locations_to_check_table(
    ...
    for i, user in enumerate(result["上架员"].tolist()):
        eligible_mask = (pool_user != user) & (~pool_loc_norm.isin(excluded_locations))
...
with st.sidebar:
    st.header("上传文件")
    source_upload = st.file_uploader("1) 上传“前一日上架结果表”（Excel）", type=["xlsx"])
    diff_upload = st.file_uploader("2) 上传“差异明细表”（Excel）", type=["xlsx"])
...
run_btn = st.button("运行生成", type="primary", disabled=not (source_upload and diff_upload))
