...
def sample_locations_to_check_table(
    ...
    for i, user in enumerate(result["上架员"].tolist()):
        eligible_mask = (
            (pool_user != user)
            & (~pool_loc_norm.str.startswith(("A135", "A137")))
        )
...
with st.sidebar:
    ...
    st.markdown("### 上架量筛选范围")
    qty_min = st.number_input("上架量下限（>=）", min_value=1.0, max_value=10000.0, value=1.0, step=1.0)
    qty_max = st.number_input("上架量上限（<=）", min_value=1.0, max_value=10000.0, value=50.0, step=1.0)
